...
METHODS=('lwang.AttackGraph' 'poolsappasitmoop.BayesianAttackGraphAdapted' 'almohri.Main.Main')
...
echo $j
	...
	do
		...
		echo "$i/"
		/usr/lib/jvm/java-8-oracle/bin/java -Dfile.encoding=UTF-8 -classpath $CLASSPATH es.um.multigraph.decision.$j $i/ > stdouts/stdout_"$DIR_NAME"_"$j".txt
		wait
