...
public class SLP extends Solver {
	...
	public SLP() {
		/*
		 * Moving limit (epsilon) is used to prevent large changes in the solution. The
		 * moving limit will be applied to all variables such that |Z_i - (Z_0)_i| <=
		 * epsilon
		 */
		this.epsilon = 0.2;
		/*
		 * We will slowly decrease the epsilon by epsilonChangeFactor in each iteration.
		 */
	...
	public Graph solveMinMax(String path, Graph g) {
		int target = 18; // target node
		/*
		 * Test if the graph has an initial solution. If not abort. SLP cannot start
		 * without a warm up.
		 */
		...
		/*
		 * Y selectors need some value. This method will assign them and store them in
		 * the graph for each goal node.
		 */
		...
		for (i = 0; difference > this.differenceFactor; i++) {
			...
			/*
			 * In the following I increase the difference factor for deciding whether the
			 * loop must end. This will prevent iterating forever when solutions repeatedly
			 * keep in coming up and the difference never reaches the initially assigned
			 * factor. I did this according to the problems I tested. It might not be always
			 * true.
			 */
...
//			System.out.println("Move limit changed to " + this.differenceFactor);
	...
	private String neighborsSLPMinMax(List neighbors, Vertex n, int index) {
		...
		if (n.isGoal() && neighbors.size() == 1) {
			constraint = "s.t. c" + n.node + ": x" + n.node + " = x" + ((Vertex) neighbors.get(0)).node + ";\n";
		}
...
//            constraint += ";\n"; 
			...
			constraint += "s.t. c" + n.node + ": x" + n.node + " = " + texp.taylorExpansion(n);
			constraint += ";\n";
...
//			add here the t for improvement TODO
			else {
				double a = 1.0;
				String nonfact = "x";
				for (int i = 0; i < neighbors.size(); i++) {
...
//            constraint += ";\n"; 
	...
	/*
	 * Prepare the vertex for SLP. If it is a goal node, it will need some random
	 * weights for its Y selectors. These are carefully chosen in the called method.
	 */
...
//	 * 
//	 * @param g
//	 * @return List of admissible rule nodes
//	 */
//
//	public List<Vertex> getNar(Graph g) {
